<template xmlns="http://www.w3.org/1999/html">
    <Head title="USePass - Admin Activity Logs" />
    <div class="min-h-screen bg-gray-50">
        <!-- Header Section with Controls -->
        <div class="ml-8 mr-8 rounded-[10px] bg-white shadow-sm border-b top-0 z-20">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
                    <h1 class="text-2xl font-bold text-gray-900">All Activity Logs</h1>
                                ...
                                <td colspan="5" class="px-6 py-4 text-center text-gray-500">
                                    <div class="flex items-center justify-center">
                                        <svg class="animate-spin h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3042 1.135 5.824 3 7.938l3-2.647z"></path>
                                        </svg>
                                        Loading logs...
                                    </div>
                                </td>
...
export default {
    ...
    computed: {
        ...
        filteredLogs() {
            ...
            // Only filter by selected date (removed user filtering)
    ...
    methods: {
        async fetchLogs() {
            ...
            try {
                // Removed user_id parameter to fetch all logs
                const response = await axios.get('/activity-logs', {
                    params: {
                        per_page: 1000, // Fetch more records to show all logs
                        all_users: true // Optional: Add this parameter to explicitly request all users' logs
                    }
                });

                if (response.data.success) {
                    this.logs = response.data.data.data || response.data.data;
                    this.totalLogs = this.logs.length; // Show total count of all logs
                } else {
